...
def sort_files():
   folder = fd.askdirectory(title='Select the folder to sort files')
   ...
   for file in files:
      if os.path.isfile(os.path.join(folder, file)):
         ...
         folder_path = os.path.join(folder, extension_1)
         if not os.path.exists(folder_path):
            os.makedirs(folder_path)
         shutil.move(os.path.join(folder, file), os.path.join(folder_path, file))

   mb.showinfo(title="Files Sorted", message='Files have been sorted successfully')


title = '''File Explorer - Lalit, Atharva, Mayank !'''
background = 'White'

button_font = ("Times New Roman", 13)
button_background = 'Lavender'
...
Label(root, text=title, font=("Comic Sans MS", 15), bg=background, wraplength=250).place(x=130, y=0)

Button(root, text='Open a file', width=20, font=button_font, bg=button_background, command=open_file).place(x=130, y=70)

Button(root, text='Copy a file', width=20, font=button_font, bg=button_background, command=copy_file).place(x=130, y=110)

Button(root, text='Rename a file', width=20, font=button_font, bg=button_background, command=rename_file).place(x=130, y=150)

Button(root, text='Delete a file', width=20, font=button_font, bg=button_background, command=delete_file).place(x=130, y=190)

Button(root, text='Open a folder', width=20, font=button_font, bg=button_background, command=open_folder).place(x=130, y=230)

Button(root, text='Delete a folder', width=20, font=button_font, bg=button_background, command=delete_folder).place(x=130, y=270)

Button(root, text='Move a folder', width=20, font=button_font, bg=button_background, command=move_folder).place(x=130, y=310)

Button(root, text='List all files in a folder', width=20, font=button_font, bg=button_background,
      command=list_files_in_folder).place(x=130, y=350)

Button(root, text='Sort files in a folder', width=20, font=button_font, bg=button_background, command=sort_files).place(x=130, y=390)
